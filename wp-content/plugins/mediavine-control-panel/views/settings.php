...
<form method="post" action="options.php">
	...
	<?php if ( Upstream::has_launch_mode_changed() && current_user_can( 'manage_options' ) ) { ?>
	...
	<div class="option-group launch-mode-group" id="mv_control_launch_mode">
		<section class="mvoption">
			<div class="option">
				<a class="button button-secondary" id="mv_refresh_launch_mode" href="?page=mediavine_amp_settings&refresh_launch_status=1">Refresh Mediavine Launch Status</a>
			</div>
		...
		<section class="mvoption">
			<div class="option">
				<a class="button button-secondary" id="mv_disable_launch_mode">Disable Launch Mode</a>
			</div>
			<div class="description">
	...
	<?php if ( $amp->hasAMP() && ! Upstream::is_launch_mode_enabled() ) { ?>
		<div class="option-group">
			<h3>AMP Settings</h3>
		...
		<div class="option-group">
			...
			<section class="mvoption">
				<?php if ( $this->hasAMPForWP() && ! Upstream::is_launch_mode_enabled() ) { ?>
	...
	if ( $this->has_web_stories() && ! Upstream::is_launch_mode_enabled() ) {
		?>
		<div class="option-group">
			<h3>AMP Web Stories Settings</h3>
			<section class="mvoption">
				<div class="option">
					<input id="<?php echo esc_attr( $amp->get_key( 'enable_web_story_ads' ) ); ?>" name="<?php echo esc_attr( $amp->get_key( 'enable_web_story_ads' ) ); ?>"
						<?php checked( true === $amp->option( 'enable_web_story_ads' ) ); ?> value="true" type="checkbox"/>
					&nbsp;<label for="<?php echo esc_attr( $amp->get_key( 'enable_web_story_ads' ) ); ?>">Enable AMP Web Story Ads</label>
				</div>
				<div class="description">
	...
	<?php if ( $this->option( 'site_id' ) && ! Upstream::is_launch_mode_enabled() ) { ?>
		<div class="option-group">
			<h3>Advanced</h3>
			<section class="mvoption">
...
<script type="text/javascript">
	window.addEventListener('load', () => {
		var button = document.getElementById( 'mv_disable_launch_mode' )
		if ( ! button ) {
			return;
		}
		button.addEventListener('click', (e) => {
			var confirmed = window.confirm('You are about to exit launch mode. Please only do this if instructed to by a Mediavine Support team member.')
			if ( confirmed ) {
				fetch( ajaxurl, {
					method: 'POST',
					body: new URLSearchParams({
						action: 'mv_disable_launch_mode',
						_wpnonce: '<?php echo wp_create_nonce( 'disable-launch-mode' ); // phpcs:ignore ?>',
					})
				})
				.then( response => {
					if ( ! response.ok ) {
