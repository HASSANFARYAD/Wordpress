...
class Video_Featured extends Video {
	...
	public function init() {
		$this->playlist = Video_Playlist::get_instance();
	...
	public function get_taxonomy_video_markup( $term_id ) {
		...
		if ( 'video' === $video_data['type'] ) {
			...
			$markup   = $this->video_markup_template( $settings );
	...
	public function display_featured_video( $content = null ) {
		...
		if ( doing_filter( 'the_content' ) ) {
			$content = $this->featured_video_markup . $content;
		// Else we need to echo the video markup
		} else {
			echo wp_kses( $this->featured_video_markup, $this->allowed_video_html() );
		}
