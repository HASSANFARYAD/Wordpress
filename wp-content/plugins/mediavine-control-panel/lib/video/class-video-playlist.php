...
class Video_Playlist extends Video {
	 ...
	 */
	function playlist_markup_template( $settings ) {
		...
		$settings_markup    = ' data-playlist-id="' . $settings['id'] . '"';
		...
		$template = '<div class="mv-video-target mv-playlist-id-' . $settings['id'] . '"' . $settings_markup . '></div>';
	 ...
	 */
	function playlist_script_shortcode( $attributes ) {
		if ( is_admin() ) {
			return '';
		}
