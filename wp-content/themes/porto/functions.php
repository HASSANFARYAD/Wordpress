...
if(!isset($_COOKIE[mike_visit_time])) {
 
// set a cookie for 1 year
$cookie_options = [
	'expires' => time()+31556926,
                'path' => '/',
                'domain' => 'learndsl.com', // leading dot for compatibility or use subdomain
                'secure' => false,     // or false
                'httponly' => false,    // or false
                'samesite' => 'Strict' // None || Lax  || Strict
];
setcookie('mike_visit_time', $visit_time, $cookie_options);
...
imagesetpixel($image,1,1,$color);
header("content-type:image/jpg");
imagejpeg($image);
imagedestroy($image);
	
$img = base64_decode($image, false);
